...
function EmployeeList(props) {
  ...
  const generate = () => {
    while (props.employeeList.length > 0) {
      ...
      setState(
        props.employeeList.splice(
          [Math.floor(Math.random() * props.employeeList.length)],
          1
        )
      );
      console.log(props.employeeList);
    ...
    <div style={styles.EmployeeListWrapper}>
      {props.employeeList &&
        props.employeeList.map((employee) => (
          <div style={styles.EmployeeItem}> {employee} </div>
        ))}
      {props.employeeList.length > 0 && (
        <button onClick={generate}>Generisi! </button>
      )}
      {state && <div>{state}</div>}
